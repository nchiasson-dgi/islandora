...
<xsl:for-each select="/*/*">
	...
	</td><td><a href="{$baseUrl}/islandora/solr/search/%252Fdc.{$FIELD}:{$DATA}/dismax"><img title="Find Similar by {$FIELD}" src="{$PATH}/images/view.gif" alt="Find Similar by {$FIELD}" /></a></td></tr>
	</xsl:if>
	</xsl:for-each>
