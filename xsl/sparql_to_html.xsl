...
<xsl:stylesheet xmlns:s="http://www.w3.org/2001/sw/DataAccess/rf1/result" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"> 
    ...
    <xsl:variable name="thisTitle" select="$collectionTitle"></xsl:variable> 
    <xsl:variable name="size" select="15"></xsl:variable> 
    ...
    <xsl:variable name="cellsPerRow" select="3"></xsl:variable> 
    <xsl:variable name="count" select="count(s:sparql/s:results/s:result)"></xsl:variable> 
    <xsl:template match="/"> 
        <xsl:if test="$count&gt;0"> 
            <table cellpadding="3" cellspacing="3" class="mapgrid" width="90%"> 
                <tr> 
                                ...
                                <xsl:when test="$end &gt;= $count"> 
                                    ...
                                    <a> 
                                        <xsl:attribute name="href"> 
                                            <xsl:value-of select="$BASEURL"></xsl:value-of>/fedora/repository/<xsl:value-of select="$thisPid"></xsl:value-of>/-/<xsl:value-of select="$thisTitle"></xsl:value-of>/<xsl:value-of select="$page - 1"></xsl:value-of> 
                                        </xsl:attribute> 
                                        &lt;&lt;Prev
                                    </a> 
                                </xsl:when> 
            ...
            <br clear="all"></br> 
            <div STYLE="text-align: center;" class="collection"> 
                <xsl:choose> 
                    <xsl:when test="$end &gt;= $count and $start = 1"> 
                        <xsl:value-of select="$start"></xsl:value-of>-<xsl:value-of select="$count"></xsl:value-of> 
                        of <xsl:value-of select="$count"></xsl:value-of> <br></br> 
